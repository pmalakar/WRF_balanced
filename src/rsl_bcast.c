...
RSL_LITE_TO_CHILD_INFO ( Fcomm, msize_p,
                         ...
                         icoord_p, jcoord_p,
                         idim_cd_p, jdim_cd_p,
			 nest_comm, nest_ntasks_x, nest_ntasks_y,       	/* SIB_BEGIN nest parameters added */
			 nest_start_rank, nest_start_row, nest_start_col,       /* nest parameters added SIB_END */
                         ig_p, jg_p,
                         retval_p )
...
/*SIB_BEGIN*/
  ...
  int myrank, nest_comm_size, comm_size;
  int local_x, local_y, local_Px, local_Py;
  int nest_rank, nest_row, nest_col; 
  MPI_Comm *local_nest_comm, dummy_nest_comm;   /* local sub communicator for less number of processes */  
...
/*SIB_BEGIN*/
    ...
    local_nest_comm = &dummy_nest_comm ;
    *local_nest_comm = MPI_Comm_f2c( *nest_comm );
    local_x = *nest_ntasks_x;
    local_y = *nest_ntasks_y;
    nest_rank = *nest_start_rank;
    nest_row = *nest_start_row;
    nest_col = *nest_start_col;
...
/*SIB_BEGIN*/
  	   TASK_FOR_POINT ( &ni, &nj, nids_p, nide_p, njds_p, njde_p, &local_x, &local_y, &local_Px, &local_Py, min_subdomain, min_subdomain, &ierr );
//         coords[1] = local_Px ; coords[0] = local_Py ;
           coords[1] = local_Px + nest_col - 1 ; coords[0] = local_Py + nest_row - 1 ; /*start_rank 9 = r2 c1*/
           //coords[1] = local_Px + nest_col ; coords[0] = local_Py + nest_row ; /*start_rank 1 = 1,0 ; 2 = 2,0 ; 5 = 1,1*/
	   if(MPI_Cart_rank(*comm, coords, &P) != MPI_SUCCESS) {
	   	printf("\nSIB_DEBUG: RSL_LITE_TO_CHILD_INFO: error in MPI_Cart_rank\n");
	   }
/*SIB_END*/
